        ...
        xP.controls.register({name: 'date_picker', base: '_secret', prototype: {
                ...
                init: function(params){
                    ...
                    /*if (params.min !== undefined){
                        this.min = new Date(params.min);
                    }*/

                    this.restricted_dates = params.restricted_dates || [];
                    this.min_hour = params.min_hour || 0;
                    this.max_hour = params.max_hour || 23;

                    for(var i = this.min_hour; i < this.max_hour + 1; i++){
			...
			*/
                ...
                checkDate: function(date, min, max) {
                    if (min > date || max < date) {
                        return false;
                    }

                    if (this.only_workdays && (date.getDay() === 0 || date.getDay() === 6)) {
                        return false;
                    }

                    if (this.restricted_dates.map(Number).indexOf(+date) !== -1) {
                        return false;
                    }
                ...
                build: function(d, update_value){
                    ...
                    for(var i = 0; i < first_day; i++){
                        ...
                        if (!this.checkDate(td_date, _min, _max)) {
                            class_name += ' invalid'
                        }
